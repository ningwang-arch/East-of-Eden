...
for i in range(1, 13):
    ...
    list_html = list_response.content.decode()  # 获取每列表的网页源代码
    # print(list_html)
    ...
    name = element_html.xpath(
        "//div[@class='item-link-text']//text()")  # 提取每列表专辑名称
    number = element_html.xpath(
        "//span[@class='item-num']//text()")  # 提取每列表专辑图片数量
